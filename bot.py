...
# ================== Mini App'dan kelgan ma'lumotlar ==================
...
def handle_webapp_data(message):
    ...
    if action in state_map:
        state, msg_key = state_map[action]
        user_state[uid] = state
        bot.send_message(message.chat.id, t(msg_key, lang))
        return
    if action == "genpass":
        chars = string.ascii_letters + string.digits + "!@#$%^&*"
        pwd = "".join(random.choice(chars) for _ in range(16))
        bot.send_message(message.chat.id, t("genpass_result", lang, pwd=pwd))
        return
    if action == "2fa":
        bot.send_message(message.chat.id, t("2fa_text", lang))
        return
    if action == "stats":
        data = load_data()
        user = data["users"].get(str(uid), {})
        bot.send_message(message.chat.id, t("stats_header", lang) + f"\n\n🔗{user.get('url_checks',0)} 🌐{user.get('domain_checks',0)} 🌍{user.get('ip_checks',0)} 🔓{user.get('pass_checks',0)}")
        return
    if action == "emergency":
        bot.send_message(message.chat.id, t("emergency_text", lang))
        return
